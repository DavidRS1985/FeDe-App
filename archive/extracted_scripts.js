...
// line 2572
        ...
        window.addEventListener('error', function(e) {
            window._fedeErrors.push(e.message || e.toString());
            var eb = document.getElementById('error-box');
            if (!eb) {
                eb = document.createElement('div');
                eb.id = 'error-box';
                eb.style.cssText = 'position:fixed;top:10px;left:10px;right:10px;background:rgba(220,38,38,0.9);color:white;padding:15px;z-index:999999;border-radius:8px;font-family:sans-serif;font-weight:bold;font-size:12px;box-shadow:0 10px 25px rgba(0,0,0,0.5);overflow-wrap:break-word;';
                if(document.body) document.body.appendChild(eb);
            }
            eb.innerHTML = 'CRITICAL ERROR:<br>' + window._fedeErrors.join('<br>') + '<hr>File: ' + e.filename + '<br>Line: ' + e.lineno;
            
            if (e.filename && e.lineno) {
                fetch(e.filename).then(res => res.text()).then(text => {
                    var lines = text.split('\n');
                    var badLine = lines[e.lineno - 1];
                    var prevLine = lines[e.lineno - 2];
                    var nextLine = lines[e.lineno];
                    eb.innerHTML += '<hr>CODE:<br><pre style="background:#000;color:#0f0;padding:5px;overflow:auto;">' + 
                      (e.lineno - 1) + ': ' + prevLine + '\n' +
                      e.lineno + ': ' + badLine + '\n' +
                      (e.lineno + 1) + ': ' + nextLine +
                    '</pre>';
                }).catch(err => console.log("Can't fetch script:", err));
            }
        });
        window.addEventListener('unhandledrejection', function(e) {
            window._fedeErrors.push('Promise: ' + (e.reason ? e.reason.toString() : 'Unknown'));
            var eb = document.getElementById('error-box');
            if (eb) eb.innerHTML = 'CRITICAL PROMISE ERROR:<br>' + window._fedeErrors.join('<br>');
        });
...
// line 3365
    ...
    const PHRASES = [
        "RETICULANDO ESTANTES...", "CONTANDO GRANOS DE ARROZ...", "PULIENDO COPAS...", 
        "CALIBRANDO MERMAS...", "INVOCANDO REPARTIDOR...", "BUSCANDO TUPPER PERDIDO...",
        "AMASANDO EL SERVIDOR...", "COSECHANDO TOMATES...", "EQUILIBRANDO SAL..."
    ];
    ...
    function initApp() {
        ...
        console.log('🚀 [BOOT] Iniciando FeDe...');

        // ⏱️ TIMEOUT MASTER — Unico, garantizado, nunca cancelado
        // Si despues de 7s nada resolvió, forzamos la carga de todas formas
        window._masterBootTimeout = setTimeout(() => {
            console.error('🔴 [BOOT] TIMEOUT MASTER - Forzando carga');
            hideLoader('master-timeout');
            toast('⚠️ Conexión lenta — usando datos locales');
        }, 7000);
        ...
        try {
            ...
            window.addEventListener('online', () => { if (typeof processOfflineQueue === 'function') processOfflineQueue(); });

            // Usar snapshot local mientras llega la respuesta del servidor
            loadLocalSnapshot();

            // Llamar al servidor GAS
            console.log('📡 [BOOT] Conectando con GAS...');
            loadConfigFromGAS();
            ...
            setTimeout(() => {
                try {
                    ...
                    console.warn('⚠️ HolidayAlert no disponible:', e.message);
                }
            }, 2000);

        } catch (e) {
            console.error('🔴 [BOOT] Error crítico:', e);
            hideLoader('boot-error');
            toast('❌ Error en inicialización');
        }
    }

    // Manejo robusto del disparo de carga - Evita doble ejecución
    ...
    function updateTime() {
        ...
        const dayEl = document.getElementById('hdr-day'), homeDayEl = document.getElementById('home-day');
    ...
    function setSyncStatus(st) {
        ...
        if (icon) {
            icon.setAttribute('data-lucide', st === 'online' ? 'cloud-check' : (st === 'pending' ? 'cloud-upload' : 'cloud-off'));
            refreshIcons();
        }
    }
    ...
    function loadPredictions() { 
        // Con timeout y error handler
        google.script.run
            .withSuccessHandler(r => { 
                if (r && r.success) { 
                    PREDICTIONS = {}; 
                    (r.predictions || []).forEach(p => PREDICTIONS[p.name] = p.suggestion); 
                    renderProducts(); 
                }
            })
            .withFailureHandler(e => {
                console.warn('❌ Predicciones no disponibles:', e);
                // Continuar sin predicciones
                renderProducts();
            })
            .withUserObject({ timeout: 3000 })
            .getPredictiveAnalysis(activeSheetId); 
    }

    function loadConfigFromGAS() {
        updateDBLabel();
        google.script.run.withSuccessHandler(onConfigLoaded).withFailureHandler(onConfigFailed).getAppConfig(activeSheetId);
    }

    function onConfigLoaded(result) {
        // Limpiar TODOS los timeouts al recibir respuesta del servidor
        if (window._masterBootTimeout) { clearTimeout(window._masterBootTimeout); window._masterBootTimeout = null; }
        if (window.loaderTimeout) { clearTimeout(window.loaderTimeout); window.loaderTimeout = null; }
        hideLoader('config-loaded');

        if (!result) {
            console.warn('GAS payload nulo');
            toast("⚠️ Modo Offline - Datos locales");
            return;
        }

        try {
            const { config, history, ssName, isOfflineMode, source, error } = result;
            
            // Actualizar nombre de DB
            if (ssName) { 
                dbName = isOfflineMode ? `${ssName} (Offline)` : ssName;
                updateDBLabel(); 
            }

            // Cargar datos - Funciona en línea o sin conexión
            if (config) {
                if (config.cats) CATS = config.cats;
                if (config.favs) FAVS = config.favs;
            ...
            // Cache local para inicio instantáneo en la próxima carga
            localStorage.setItem(`fede_snapshot_${activeSheetId}`, JSON.stringify({ 
                cats: CATS, favs: FAVS, history: HIST, units: units, custom_units: CUSTOM_UNITS, ts: Date.now() 
            }));

            // Mostrar estado
            if (isOfflineMode) {
                console.warn('⚠️ Modo Offline: ', error);
                toast("📡 Modo Offline - Últimos datos");
            } else {
            ...
            console.error('Data proc error:', e);
            toast("⚠️ Error procesando datos - Usando offline");
            // Intenta renderizar aunque falle
            try {
                renderCatGrid();
                renderTree();
                setTimeout(() => {
                ...
                console.error('Error crítico en renderizado:', e2);
            }
        }
    }
    ...
    function onConfigFailed(error) {
        ...
        // Limpiar TODOS los timeouts y ocultar el loader
        if (window._masterBootTimeout) { clearTimeout(window._masterBootTimeout); window._masterBootTimeout = null; }
        if (window.loaderTimeout) { clearTimeout(window.loaderTimeout); window.loaderTimeout = null; }
        hideLoader('config-failed');
    ...
    function hideLoader(reason = '') {
        // Cancelar el master timeout si la carga fue exitosa
        if (window._masterBootTimeout) {
            clearTimeout(window._masterBootTimeout);
            window._masterBootTimeout = null;
        }
        ...
        // Si ya está oculto, no hacer nada más
        if (loader.style.display === 'none') return;
        
        console.log('✓ [LOADER] Ocultando loader:', reason);
        ...
        // Ocultar sin animación en timeouts
    ...
    function startLoadingPhrases() {
        ...
        const update = () => {
            ...
            el.style.animation = 'phraseFade 0.5s ease-in-out';
            el.textContent = randomPhrase;
        };

        update(); // First one immediate
        // En móvil, cambiar frase más lentamente
        const isMobile = /iPhone|iPad|Android|Mobile/.test(navigator.userAgent);
        loadingInterval = setInterval(update, isMobile ? 2000 : 1500);
    }
    ...
    const MODE_CONFIG = {
        pedido:    { label: 'PEDIDO',    cls: 'pedido',    bodyClass: 'mode-pedido',    borderColor: 'var(--green)',  glow: 'rgba(16,185,129,0.15)' },
        stock:     { label: 'STOCK',     cls: 'stock',     bodyClass: 'mode-stock',     borderColor: 'var(--warning)',glow: 'rgba(245,158,11,0.15)' },
        recepcion: { label: 'RECEPCIÓN', cls: 'recepcion', bodyClass: 'mode-recepcion', borderColor: 'var(--blue)',   glow: 'rgba(59,130,246,0.15)' },
    };
    ...
    function switchModeTo(m) {
        ...
        closeModeSwitcher();
        if (m === 'recepcion') {
            // La vista de recepción ahora está integrada en el flujo o tiene su propia lógica
            enterMode('recepcion');
            goTab('relevamiento'); // Integrado al relevamiento para consistencia
        } else {
            enterMode(m);
            goTab('relevamiento');
        }
    ...
    function genProductRow(p, prodUnits) {
        ...
        const is86 = d.qty === 86;
        const unitChips = prodUnits.map(u => {
            const isActive = d.unit === u ? 'active' : '';
            return '<div class="uchip ' + isActive + '" data-prod="' + ep + '" data-unit="' + encodeURIComponent(u) + '" onclick="setUnitEl(this)" style="padding:8px 14px; font-size:12px; min-width:60px;">' + u + '</div>';
        }).join('');

        const predHtml = PREDICTIONS[p] ? '<div style="font-size:10px; color:var(--blue); font-weight:800; margin-top:2px; text-transform:uppercase; letter-spacing:0.5px;">Sug: ' + PREDICTIONS[p] + ' ' + d.unit + '</div>' : '';
    ...
    const UNIT_MAPPING = {
        ...
        "Verdurería": ["kg", "atado/s", "unidad/es"], // BUG 12 FIX: era "Verduras"
    ...
    function getUnitsForProduct(catName, prodName = '') {
        ...
        // 🚀 INTELIGENCIA DE UNIDADES (SISTEMA DE MATCHING DINÁMICO)
        // -------------------------------------------------------------
        
        // 1. Carnes, Pescados y Embutidos
        if (p.includes('carne') || p.includes('pollo') || p.includes('cerdo') || p.includes('bife') || p.includes('lomo') || 
            p.includes('bondiola') || p.includes('peceto') || p.includes('oso') || p.includes('panceta') || 
            p.includes('pescado') || p.includes('merluza') || p.includes('salmón') || p.includes('salmon') || p.includes('pechuga')) {
            return ['kg', 'pieza/s', 'unidad/es', 'gramos'];
        } 
        
        // 2. Lácteos y Quesos
        if (p.includes('queso') || p.includes('sardo') || p.includes('gouda') || p.includes('parmesano') || p.includes('brie') || 
            p.includes('azul') || p.includes('horma')) {
            return ['kg', 'horma/s', 'unidad/es', 'gramos'];
        }
        if (p.includes('leche') || p.includes('crema') || p.includes('yogurt') || p.includes('miel') || p.includes('aceite')) {
            return ['litro/s', 'caja/s', 'pote/s', 'botella/s', 'bidón/es'];
        }
        if (p.includes('burrata') || p.includes('manteca') || p.includes('ricota') || p.includes('provoleta')) {
            return ['unidad/es', 'pote/s', 'kg', 'pan/es'];
        }
        
        // 3. Verdulería y Frutas
        if (p.includes('papa') || p.includes('cebolla') || p.includes('calabaza') || p.includes('zanahoria') || p.includes('tomate')) {
            return ['kg', 'cajón/es', 'bolsa/s', 'unidad/es'];
        }
        if (p.includes('lechuga') || p.includes('rúcula') || p.includes('espinaca') || p.includes('rúcula') || p.includes('perejil') || p.includes('verdeo')) {
            return ['atado/s', 'kg', 'bolsa/s', 'unidad/es'];
        }
        if (p.includes('limón') || p.includes('manzana') || p.includes('naranja') || p.includes('palta') || p.includes('choclo') || p.includes('ajo')) {
            return ['kg', 'unidad/es', 'caja/s'];
        }
        if (p.includes('arándano') || p.includes('frutilla') || p.includes('cherry')) {
            return ['paquete/s', 'bandeja/s', 'kg', 'gramos'];
        }

        // 4. Panadería y Masas
        if (p.includes('pan ') || p.includes('brioche') || p.includes('focaccia') || p.includes('prepiz') || p.includes('tequeño')) {
            return ['unidad/es', 'bolsa/s', 'kg', 'docena'];
        }

        // 5. Pastas y Secos
        if (p.includes('fideo') || p.includes('spaguetti') || p.includes('pasta') || p.includes('tallarine') || p.includes('ñoqui') || p.includes('sorrentino') || 
            p.includes('harina') || p.includes('azúcar') || p.includes('arroz') || p.includes('fécula') || p.includes('lenteja') || p.includes('garbanzo')) {
            return ['kg', 'paquete/s', 'bolsa/s', 'gramos'];
        }

        // 6. Condimentos y Conservas
        if (p.includes('sal ') || p.includes('pimienta') || p.includes('condimento') || p.includes('ají') || p.includes('orégano') || p.includes('vinagre') || p.includes('aceto')) {
            return ['paquete/s', 'frasco/s', 'kg', 'gramos', 'botella/s'];
        }
        if (p.includes('aceituna') || p.includes('alcaparra') || p.includes('hongo') || p.includes('tomates de') || p.includes('pasta de') || p.includes('mermelada')) {
            return ['frasco/s', 'lata/s', 'kg', 'unidad/es'];
        }

        // 7. Bebidas y Varios
        if (p.includes('vino') || p.includes('cerveza') || p.includes('gaseosa') || p.includes('agua') || p.includes('jugo')) {
            return ['botella/s', 'cajón/es', 'litro/s', 'unidad/es'];
        }
        if (p.includes('huevo')) return ['maple', 'docena', 'unidad/es'];
        
        // 8. Platos de Carta / Cocina / Personal
        if (catName === 'Platos carta' || catName === 'Personal' || p.includes('milanesa') || p.includes('pizza') || p.includes('tarta') || p.includes('hamburguesa')) {
            return ['porción/es', 'unidad/es', 'kg', 'bandeja/s'];
        }
    ...
    function renderFAB() {
        ...
        if (!fab) return; // BUG 7 FIX: null guard
        if (count > 0) {
    ...
    function openResumen() {
        ...
        // Agregar botones de acción según el modo
        let actionButtons = '';
        if (currentMode === 'pedido') {
            actionButtons = `
            <div style="display:flex; gap:10px; margin-top:20px;">
                <button onclick="generateComandaFromRelData()" style="flex:1; padding:12px; background:linear-gradient(135deg, var(--primary), #FB923C); color:white; border:none; border-radius:8px; cursor:pointer; font-weight:700; transition:all 0.2s;">
                    📋 COMANDA
                </button>
                <button onclick="sendWA()" style="flex:1; padding:12px; background:linear-gradient(135deg, var(--green), #34D399); color:white; border:none; border-radius:8px; cursor:pointer; font-weight:700; transition:all 0.2s;">
                    💬 WhatsApp
                </button>
            </div>`;
            ...
            <div style="display:flex; gap:10px; margin-top:20px; flex-wrap:wrap;">
                <button onclick="loadLastPedido()" style="flex:1; min-width:100px; padding:12px; background:var(--blue); color:white; border:none; border-radius:8px; cursor:pointer; font-weight:700;">
                    📦 Cargar Pedido
                </button>
                <button onclick="compareReceiptWithPedido()" style="flex:1; min-width:100px; padding:12px; background:var(--warning); color:white; border:none; border-radius:8px; cursor:pointer; font-weight:700;">
                    🔍 Comparar
                </button>
            </div>`;
        }
    ...
    /* ── MÓDULO RECEPCIÓN + COMANDA ── */
    
    /**
     * Carga el último pedido como base para la recepción
     */
    function loadLastPedido() {
        google.script.run
            .withSuccessHandler(res => {
                if (!res.success || !res.pedido) {
                    toast('⚠️ No hay pedidos previos para cargar');
                    return;
                }
                
                const lastPedido = res.pedido;
                toast(`✓ Pedido cargado: ${new Date(lastPedido.fecha).toLocaleDateString('es-AR')}`);
                
                // BUG 4 FIX: Guardar el id del último pedido para comparación posterior
                localStorage.setItem('fede_last_pedido_id', lastPedido.id);
                
                // Pre-llenar relData con items del último pedido
                relData = {};
                if (Array.isArray(lastPedido.items)) {
                    lastPedido.items.forEach(item => {
                        if (item.prod) {
                            relData[item.prod] = {
                                qty: item.qty,
                                unit: item.unit,
                ...
                localStorage.setItem('fede_draft', JSON.stringify(relData));
                renderFAB();
                renderProducts();
            })
            .withFailureHandler(e => toast('❌ Error: ' + e.message))
            .getLatestPedido(activeSheetId);
    }

    /**
     * Compara recepción actual vs pedido anterior
     */
    function compareReceiptWithPedido() {
        const receiptItems = Object.entries(relData).map(([prod, data]) => ({
            prod,
            qty: data.qty,
            unit: data.unit,
            cat: data.cat
        ...
        google.script.run
            .withSuccessHandler(res => {
                if (!res.success) {
                    toast('❌ Error en comparación: ' + res.error);
                    return;
                }
                
                const status = res.status;
                const diff = res.differences || [];
                
                let html = `<div style="font-size:13px; padding:20px;">
                    <div style="margin-bottom:16px; padding:12px; background:${
                        status === 'completa' ? 'rgba(16,185,129,0.1)' : 'rgba(239,68,68,0.1)'
                    }; border-radius:8px; border-left:3px solid ${
                        status === 'completa' ? 'var(--green)' : 'var(--red)'
                    };">
                        <strong>Estado: ${status.toUpperCase()}</strong><br/>
                        ${diff.length} diferencia${diff.length !== 1 ? 's' : ''} detectada${diff.length !== 1 ? 's' : ''}
                    </div>
                    ${diff.length > 0 ? `
                        <div style="background:rgba(255,255,255,0.02); padding:12px; border-radius:8px; max-height:300px; overflow-y:auto;">
                            ${diff.map(d => `
                                <div style="margin-bottom:8px; padding:8px; border-bottom:1px dashed rgba(255,255,255,0.1);">
                                    <strong>${d.prod}</strong><br/>
                                    Pedido: <span style="color:var(--dim);">${d.pedidoQty}${d.unit}</span> → 
                                    Recibido: <span style="color:${d.diff < 0 ? 'var(--red)' : 'var(--green)'};">${d.receiptQty}${d.unit}</span>
                                </div>
                            `).join('')}
                        </div>
                    ` : '<div style="text-align:center; color:var(--green); padding:20px;">✓ Todo coincide perfectamente</div>'}
                </div>`;
                
                const sheet = document.getElementById('sheet-overlay');
                document.getElementById('sh-title').textContent = 'Comparación Recepción';
                document.getElementById('sh-body').innerHTML = html;
                sheet.classList.add('open');
            })
            .withFailureHandler(e => toast('❌ Error: ' + e.message))
            .compareReceiptToPedido(localStorage.getItem('fede_last_pedido_id') || '', receiptItems, activeSheetId);
    }

    /**
     * Genera COMANDA formateada para WhatsApp desde relData
     */
    function generateComandaFromRelData() {
        const items = Object.entries(relData).map(([prod, data]) => ({
            prod,
            qty: data.qty,
            unit: data.unit,
            cat: data.cat || 'Diversos'
        }));
        
        google.script.run
            .withSuccessHandler(res => {
                if (!res.success) {
                    toast('❌ Error generando comanda');
                    return;
                }
                
                // Guardar en window para copiar
                window.currentComanda = res.comanda;
                
                // Mostrar preview
                const previewHtml = `<div style="font-family:monospace; font-size:12px; white-space:pre-wrap; color:var(--text2); background:rgba(0,0,0,0.3); padding:16px; border-radius:8px; max-height:400px; overflow-y:auto; line-height:1.6;">
${res.comanda}
                </div>
                <div style="margin-top:16px; display:flex; gap:10px;">
                    <button onclick="copyComandaToClipboard()" style="flex:1; padding:12px; background:var(--primary); color:white; border:none; border-radius:8px; cursor:pointer; font-weight:700;">
                        📋 Copiar
                    </button>
                    <button onclick="sendComandaViaWA()" style="flex:1; padding:12px; background:var(--success); color:white; border:none; border-radius:8px; cursor:pointer; font-weight:700;">
                        📱 WhatsApp
                    </button>
                </div>`;
                
                const sheet = document.getElementById('sheet-overlay');
                document.getElementById('sh-title').textContent = 'COMANDA GENERADA';
                document.getElementById('sh-body').innerHTML = previewHtml;
                sheet.classList.add('open');
                toast('✓ Comanda generada');
            })
            .withFailureHandler(e => toast('❌ Error: ' + e.message))
            .generateComanda(items);
    }

    /**
     * Copia comanda al clipboard
     */
    function copyComandaToClipboard() {
        if (!window.currentComanda) {
            toast('❌ No hay comanda para copiar');
            return;
        }
        
        navigator.clipboard.writeText(window.currentComanda).then(() => {
            toast('✓ Comanda copiada al clipboard');
        }).catch(err => {
            toast('❌ Error al copiar: ' + err.message);
        });
    }

    /**
     * Envía comanda por WhatsApp
     */
    function sendComandaViaWA() {
        if (!window.currentComanda) return;
        window.open(`https://wa.me/?text=${encodeURIComponent(window.currentComanda)}`, '_blank');
        toast('✓ Abriendo WhatsApp');
    }

    /**
     * Carga dashboard de analytics simplificado - SIMPLIFICADO PARA EVITAR LOOPS
     */
    function loadAnalyticsData() {
        // Solo cargamos si la función existe
        if (typeof google === 'undefined' || !google.script) return;
        
        try {
            google.script.run
                .withSuccessHandler(res => {
                    // Cargar múltiples datos en paralelo (las funciones ya existen en backend)
                    if (!res || !res.success) return;
                    // Insertar datos en el dashboard sin llamadas adicionales
                })
                .withFailureHandler(e => console.warn('Analytics fallback:', e.message))
                .getAlerts(activeSheetId);
        } catch(e) {
            console.warn('loadAnalyticsData error:', e.message);
        }
    }
        ...
        google.script.run
            .withSuccessHandler(r => {
                ...
                google.script.run
                    ...
                    .withFailureHandler(e => {
                        console.warn('Predicciones no disponibles:', e.message);
                        const intelEl = document.getElementById('dash-intelligence-list');
                        if (intelEl) intelEl.innerHTML = '<p style="color:var(--dim); font-size:12px;">Sin datos de predicciones.</p>';
                    })
                ...
                // 🚨 HOLIDAY STOCK SUGGESTIONS (Con validación)
                try {
                    const sug = window.holidayHelper && typeof window.holidayHelper.getSuggestedReorders === 'function' ? window.holidayHelper.getSuggestedReorders() : [];
                    ...
                    if (sug && sug.length > 0) {
                        ...
                        if (intelList) intelList.innerHTML = sugHtml + intelList.innerHTML;
                    }
                } catch(e) {
                    console.warn('❌ Error al cargar sugerencias de feriados:', e.message);
                }
                updateRecepReport(7); // Nuevo: Cargar reporte de recepción por defecto
                refreshIcons();
            })
    ...
    /**
     * Actualiza el mini-reporte de comparación de Recepción vs Pedido
     */
    function updateRecepReport(days, btn) {
        const container = document.getElementById('recep-report-container');
        if (!container) return;

        // BUG 8 FIX: Si no se pasa btn (ej. desde renderDashboard), marcar el botón 7D activo por defecto
        if (btn) {
            const btns = btn.parentNode.querySelectorAll('button');
            btns.forEach(b => b.classList.remove('active'));
            btn.classList.add('active');
        } else {
            // Sin btn, marcar el primero (7D) como activo
            const btns = document.querySelectorAll('.dash-range-btn');
            if (btns.length) {
                btns.forEach(b => b.classList.remove('active'));
        ...
        container.innerHTML = '<div style="text-align:center; padding:20px;"><div class="spinner-sm" style="margin:0 auto;"></div></div>';

        google.script.run
            .withSuccessHandler(r => {
                if (!r || !r.success) {
                    container.innerHTML = '<div style="text-align:center; padding:20px; color:var(--error); font-size:12px;">Error al cargar datos</div>';
                    return;
                }

                if (!r.report || r.report.length === 0) {
                    container.innerHTML = '<div style="text-align:center; padding:20px; color:var(--dim); font-size:12px;">Sin datos comparativos en este período.</div>';
                    return;
                }
                ...
                container.innerHTML = `
                    <div style="overflow-x:auto;">
                        <table style="width:100%; border-collapse:collapse; font-size:12px; font-family:inherit;">
                            <thead>
                                <tr style="color:var(--dim); font-weight:800; text-align:left; border-bottom:1px solid var(--bd);">
                                    <th style="padding:8px 4px; text-transform:uppercase;">Producto</th>
                                    <th style="padding:8px 4px; text-align:right; text-transform:uppercase;">Ped.</th>
                                    <th style="padding:8px 4px; text-align:right; text-transform:uppercase;">Rec.</th>
                                    <th style="padding:8px 4px; text-align:right; text-transform:uppercase;">Dif.</th>
                                </tr>
                            </thead>
                            <tbody>
                                ${r.report.map(row => {
                                    const diff = row.diff;
                                    const diffColor = diff < 0 ? 'var(--red)' : (diff > 0 ? 'var(--blue)' : 'var(--green)');
                                    const diffIcon = diff < 0 ? '▼' : (diff > 0 ? '▲' : '✓');
                                    return `
                                    <tr style="border-bottom:1px solid rgba(255,255,255,0.05);">
                                        <td style="padding:10px 4px; font-weight:700; color:var(--text); line-height:1.2;">
                                            ${row.name}
                                            <div style="font-size:9px; color:var(--dim); font-weight:500;">Stock: ${row.stock || 0} ${row.unit}</div>
                                        </td>
                                        <td style="padding:10px 4px; text-align:right; color:var(--dim); font-weight:600;">${row.pedido}</td>
                                        <td style="padding:10px 4px; text-align:right; font-weight:700; color:var(--text);">${row.recepcion}</td>
                                        <td style="padding:10px 4px; text-align:right; font-weight:900; color:${diffColor};">
            ...
            .getComparisonReport(activeSheetId, days);
    }
    ...
    /**
     * Renderiza dashboard mejorado con analytics de recepción
     */
    function renderDashboardWithAnalytics(data) {
        const dashContent = document.getElementById('dash-content');
        if (!dashContent) return;

        // Construir HTML con nuevas tarjetas
        let analyticsHtml = '<div style="display:grid; gap:16px; margin-bottom:24px;">';

        // Card: Alertas Activas
        if (data.alerts && data.alerts.success) {
            const totalAlerts = (data.alerts.agotados?.length || 0) + (data.alerts.pendientes?.length || 0);
            analyticsHtml += `
            <div class="alert-card ${totalAlerts > 0 ? 'warning' : 'success'}">
                <div style="display:flex; align-items:center; gap:12px;">
                    <i data-lucide="${totalAlerts > 0 ? 'alert-circle' : 'check-circle'}" style="width:var(--icon-size-large); height:var(--icon-size-large); color:${totalAlerts > 0 ? 'var(--warning)' : 'var(--success)'}; flex-shrink:0;"></i>
                    <div style="flex:1;">
                        <div style="font-weight:700; font-size:13px;">Alertas Activas</div>
                        <div style="font-size:11px; color:var(--dim); margin-top:2px;">
                            ${data.alerts.agotados?.length || 0} agotados · ${data.alerts.pendientes?.length || 0} pendientes
                        </div>
                    </div>
                </div>
            </div>`;
        ...
        // Card: Stock Crítico
        if (data.stock && data.stock.success) {
            analyticsHtml += `
            <div class="alert-card ${data.stock.bajo?.length > 0 ? 'warning' : 'info'}">
                <div style="display:flex; align-items:center; gap:12px;">
                    <i data-lucide="database" style="width:var(--icon-size-large); height:var(--icon-size-large); color:${data.stock.bajo?.length > 0 ? 'var(--warning)' : 'var(--blue)'}; flex-shrink:0;"></i>
                    <div style="flex:1;">
                        <div style="font-weight:700; font-size:13px;">Stock Crítico</div>
                        <div style="font-size:11px; color:var(--dim); margin-top:2px;">
                            ${data.stock.bajo?.length || 0} productos bajo nivel · Total en stock: ${data.stock.totalItems}
                        </div>
                    </div>
                </div>
            </div>`;
        ...
        // Card: Última Semana
        if (data.last7 && data.last7.success) {
            analyticsHtml += `
            <div class="alert-card success">
                <div style="display:flex; align-items:center; gap:12px;">
                    <i data-lucide="trending-up" style="width:var(--icon-size-large); height:var(--icon-size-large); color:var(--success); flex-shrink:0;"></i>
                    <div style="flex:1;">
                        <div style="font-weight:700; font-size:13px;">Últimos 7 Días</div>
                        <div style="font-size:11px; color:var(--dim); margin-top:2px;">
                            Pedidos: ${data.last7.totalPedidos} · Recibidos: ${data.last7.totalRecepciones} · Tasa: ${data.last7.tasa}%
                        </div>
                    </div>
                </div>
            </div>`;
        ...
        // Card: Top Consumo
        if (data.top && data.top.success && data.top.top?.length > 0) {
            const topList = data.top.top.slice(0, 3).map(t => `${t.prod} (${t.count}x)`).join(' · ');
            analyticsHtml += `
            <div class="alert-card info">
                <div style="display:flex; align-items:center; gap:12px;">
                    <i data-lucide="fire" style="width:var(--icon-size-large); height:var(--icon-size-large); color:var(--primary); flex-shrink:0;"></i>
                    <div style="flex:1;">
                        <div style="font-weight:700; font-size:13px;">Top Consumo</div>
                        <div style="font-size:11px; color:var(--dim); margin-top:2px;">
                            ${topList}
                        </div>
                    </div>
        ...
        // Agregar a dashboard
        const existingAnalytics = dashContent.querySelector('[data-analytics-panel]');
        if (existingAnalytics) {
            existingAnalytics.outerHTML = `<div data-analytics-panel>${analyticsHtml}</div>`;
        } else {
            dashContent.insertAdjacentHTML('afterbegin', `<div data-analytics-panel>${analyticsHtml}</div>`);
        }
    ...
    function saveHistoryEdit() {
        const original = HIST.find(h => h.id == editingHistId);
        if (!original) return;
        // BUG 2 FIX: Preservar fecha original, borrar primero para evitar duplicados
        const entry = { ...original, items: editingHistItems, total: editingHistItems.length }; // fecha original preservada
        google.script.run
            .withSuccessHandler(() => {
                // Borrar primero, luego insertar — evita race condition con dobles
                google.script.run
                    .withSuccessHandler(() => { loadConfigFromGAS(); closeEditHistModal(); toast('✓ Actualizado'); })
                    .withFailureHandler(e => toast('❌ Error al guardar: ' + e.message))
                    .addHistoryEntry(entry, activeSheetId);
            })
            .withFailureHandler(e => toast('❌ Error al borrar: ' + e.message))
            .deleteHistoryEntry(editingHistId, activeSheetId);
    }
    function closeEditHistModal() { document.getElementById('modal-edit-hist').classList.remove('open'); }

    /* ── UTILIDADES ── */
    function toast(m, d = 2500) { const t = document.getElementById('toast'); if (!t) return; /* BUG 10 FIX */ t.textContent = m; t.classList.add('show'); setTimeout(() => t.classList.remove('show'), d); }
    ...
    function clearHistory(type) {
        ...
        google.script.run
            ...
            .deleteHistory(type, activeSheetId); // BUG 5 FIX: pasar activeSheetId
    }
    ...
    function renderFSProduct() {
        ...
        document.getElementById('prod-fs-unit-row').innerHTML = getUnitsForProduct(fsCurrentCat).map(u => {
            // BUG 9 FIX: encodeURIComponent para evitar quiebre con nombres con comillas simples
            const eu = encodeURIComponent(u);
            const ep2 = encodeURIComponent(prod);
            return `<div class="prod-fs-uchip ${d.unit === u ? 'active' : ''}" onclick="fsSetUnit(decodeURIComponent('${eu}'), decodeURIComponent('${ep2}'))">${u}</div>`;
        }).join('');
    ...
    // refreshIcons() definida en línea ~672 con soporte de parámetro root — NO DUPLICAR AQUÍ
